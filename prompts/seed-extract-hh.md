...
- When the page states the deal MECHANIC as a blanket/summary line rather than an itemized
  list ("7 items for $7 each", "$2 off all drafts", "half-price apps", "3 tacos for $9") and
  no per-item menu is provided in the pages fetched, you still can't build itemized
  `offerings` — but do NOT let that summary line evaporate. Put it in `notes`, close to
  verbatim, so the deal is visible even without a line-item breakdown (e.g.
  `notes: "7 items for $7 each"`). This is the fallback ONLY when no items are available;
  when the page also lists which items are $7, extract those as normal priced `offerings`
  instead.
...
- CONSOLIDATE only when items share ONE identical deal. A dozen appetizers each "$3 off",
  or all drafts "$2 off", → record ONE representative offering (e.g. name "Most
  appetizers", category "appetizer", discountCents 300, description "$2–$3 off most food
  items"). But DISTINCTLY PRICED items in the happy-hour section are SEPARATE offerings —
  enumerate each with its own price (e.g. "Smoked Chowder $8", "Sliders $9", "Street Tacos
  $10"), because the reader wants the actual HH menu, not a price range. Do NOT collapse
  distinct prices into "$8–$14" or otherwise summarize them away. Beyond ~8 distinct items
  in one window, consolidate only the long tail. The sourceUrl still lets readers open the
  full menu.
