...
export default async function FlagsPage() {
  ...
  const allItems: FlaggedVenue[] = rows.map((r) => ({
    venueId: r.venueId,
    name: r.name,
    cityName: r.cityName,
    ...
    note: r.note,
    parked: r.resolution === "further_review",
  }));
  const items = allItems.filter((v) => !v.parked);
  const parked = allItems.filter((v) => v.parked);
  ...
  return (
    <main className="mt-8">
      ...
      <p className="mt-2 text-text-muted">
        ...
        inactive (reversible from <code>/admin/audit</code>); <strong>Further review</strong> parks the venue
        below with a note. Every verdict is audit-logged with its flag codes — codes that keep predicting
        &ldquo;hide&rdquo; become future auto-gate rules.
      </p>
      ...
      <h2 className="mt-10 text-xl text-text-primary" style={{ fontFamily: "var(--font-serif)" }}>
        Further review ({parked.length})
      </h2>
      <p className="mt-1 text-xs text-text-muted">
        Parked for a deeper dive. Notes should capture the sourcing story — where this data came from, what
        the pipeline got or missed and why — so recurring patterns can be codified into extractor/gate rules
        (notes export with the flag-label corpus). Keep/Hide from here settles the venue; the note stays on
        its <code>data_audit</code> row.
      </p>
      <div className="mt-4 flex flex-col gap-3">
        {parked.length === 0 ? (
          <p className="text-xs text-text-muted">Nothing parked.</p>
        ) : (
          parked.map((v) => <FlagReviewRow key={v.venueId} venue={v} />)
        )}
      </div>
    </main>
  );
