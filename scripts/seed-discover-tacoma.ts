...
async function fetchNearby(
  ...
  const res = await fetch(PLACES_ENDPOINT, {
    ...
    headers: {
      ...
      // TILE CALL, not per result, so capturing the alcohol gate + hours here (≈+$1.70
      // for a Tucson-size run) deletes the per-CANDIDATE Place Details call from enrich
      // (≈-$15/big city). photos is intentionally omitted — hero capture is broken.
      "X-Goog-FieldMask":
